...
## 3. 临时自动造词模式
Rime默认的自动造词功能会自动添加大量非必要的词组，而直接关闭自动造词功能依赖手动造词又不够方便。本方案提供了一种极为便捷的临时自动造词方式：
...
## 5. 快捷输入日期、单位、希腊字母
输入对应词，获取当前日期和时间：
- `date` - 自动列出当前日期的多种格式可选输入
- `time` - 自动列出当前时间的多种格式可选输入
- `week` - 自动列出当前星期的多种格式可选输入
- `danw` - 自动列出常用单位的多种格式可选输入，目前只支持°C和°F，可自行修改lua文件添加
- `xila` - 自动列出希腊字母的小写格式
- `Xila` - 自动列出希腊字母的大写格式 